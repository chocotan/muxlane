...
2. 桌面：设置 → 填写中继 URL 并保存。同一页会显示这台电脑的机器 ID。
...
3. 手机 App（`android/`）填同一中继 URL 和机器 ID。之后用保存的 token 重连，不必再输入 ID。
...
1. 目标机器（被连的一端）：设置里填中继 URL 并保存，复制机器 ID。
2. 本机：打开连接对话框 → 认证方式选「中继」→ 目标栏填 `wss://中继地址` → 密码栏填机器 ID → 连接。
3. 成功后会保存 token（与 SSH 密码一样单独存在 `secrets.json`），重启后自动重连。
