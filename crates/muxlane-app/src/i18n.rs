...
const TRANSLATIONS: &[(&str, &str, &str)] = &[
    ("agents.found", "已找到本机命令", "Local command found"),
    ("agents.missing", "未安装 / PATH 中未找到", "Not installed / not on PATH"),
    ("agents.unmapped", "需手工配置", "Configuration required"),
    ("agents.unsupported", "本平台暂不支持检测", "Detection unsupported on this platform"),
    ("agents.invalid", "命令配置无效", "Invalid command configuration"),
    ("agents.detecting", "检测中", "Detecting"),
    ("agents.detect", "重新检测", "Detect again"),
    ("agents.refresh", "刷新官方目录", "Refresh catalog"),
    ("agents.refreshing", "目录刷新中", "Refreshing catalog"),
    ("agents.catalog_error", "目录刷新失败，保留已有目录", "Catalog unavailable; keeping previous entries"),
    ("agents.back", "返回 Agent 列表", "Back to agents"),
    ("agents.create", "创建会话", "Create session"),
    ("agents.copy", "复制安装命令", "Copy install command"),
    ("agents.docs", "官方文档", "Documentation"),
    ("agents.manual_install", "请自行执行安装命令，再重新检测。Muxlane 不会执行安装。", "Run this command yourself, then detect again. Muxlane does not install agents."),
    ("agents.configure", "请根据文档在用户级 agents.json 配置 ACP stdio 命令，然后重新检测。", "Configure an ACP stdio command in your user-level agents.json, then detect again."),
    ("agents.found_note", "仅确认可执行文件存在，未验证认证或 ACP 能力。", "Executable found; authentication and ACP support are not verified."),
    ("agents.user_command", "（用户命令，可能下载）", " (user command; may download)"),
    ("agents.custom_warning", "用户自定义命令受信任，其参数可能启动下载或其他程序。", "User commands are trusted; their arguments may download or run other programs."),
    ("common.add", "添加", "Add"),
    ...
    ("acp.terminal_unavailable", "终端输出不可用", "Terminal output unavailable"),
    ("acp.terminal_empty", "暂无终端输出", "No terminal output"),
    ("acp.terminal_failed", "读取终端输出失败", "Failed to read terminal output"),
